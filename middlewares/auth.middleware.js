  ...
  return async (req, res, next) => {
    try {
      ...
      if (!allowedRoles.includes(req.user.role)) {
        console.error(
          `Role check failed: User role ${req.user.role} not in allowed roles [${allowedRoles.join(", ")}]`,
        );
      ...
      console.log(
        `Role check passed: User ${req.user.id} with role ${req.user.role} accessing ${req.originalUrl}`,
      );
      next();
    } catch (error) {
